...
import random
import json
from object_pursuit.model.coeffnet.coeffnet import Singlenet
...
def test_unit(data_dir, z_dir, hypernet_path, backbone_path, test_size=400, start_index=0):
    ...
    if test_size is not None:
        eval_sampler = sampler.SubsetRandomSampler(
            indices[:min(n_size, test_size)])
    else:
        eval_sampler = sampler.SubsetRandomSampler(indices[:n_size])
    eval_loader = DataLoader(eval_dataset, batch_size=64, shuffle=False,
                             num_workers=8, pin_memory=True, drop_last=True, sampler=eval_sampler)
    ...
    with open(obj_info, 'r') as f:
        ...
        for obj in obj_info:
            if obj["acc"] > max_threshold:
                ...
                test_acc, test_index, test_zf = test_unit(
                    data_dir, z_dir, hypernet_path, backbone_path)
...
def test_unseen_obj(obj_dir, z_dir, hypernet_path, backbone_path, log_name, threshold=0.7):
    ...
    for data_dir in data_dirs:
        ...
        test_acc, test_index, test_zf = test_unit(
            data_dir, z_dir, hypernet_path, backbone_path, start_index=31)
        total_count += 1
        if test_acc > threshold:
            seen_count += 1
