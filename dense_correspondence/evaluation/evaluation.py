...
from dense_correspondence.network.op_dense_correspondence_network import OPMultiDenseCorrespondenceNetwork
...
class DCNEvaluationPandaTemplate(PandaDataFrameWrapper):
    ...
    def __init__(self):
        PandaDataFrameWrapper.__init__(
            self, DCNEvaluationPandaTemplate.columns)
...
class DCNEvaluationPandaTemplateAcrossObject(PandaDataFrameWrapper):
    ...
    def __init__(self):
        PandaDataFrameWrapper.__init__(
            self, DCNEvaluationPandaTemplateAcrossObject.columns)
...
class SIFTKeypointMatchPandaTemplate(PandaDataFrameWrapper):
    ...
    def __init__(self):
        PandaDataFrameWrapper.__init__(
            self, SIFTKeypointMatchPandaTemplate.columns)
...
class DenseCorrespondenceEvaluation(object):
    ...
    Network = DenseCorrespondenceNetwork
    ...
    def load_network_from_config(self, name):
        ...
        path_to_network_params = utils.convert_data_relative_path_to_absolute_path(
            path_to_network_params, assert_path_exists=True)
        model_folder = os.path.dirname(path_to_network_params)

        dcn = self.Network.from_model_folder(
            model_folder, model_param_file=path_to_network_params)
        dcn.eval()
        return dcn
    ...
    def load_dataset_for_network(self, network_name):
        ...
        if network_name not in self._config["networks"]:
            raise ValueError("Network %s is not in config file" %
                             (network_name))

        network_folder = os.path.dirname(
            self._config["networks"][network_name]["path_to_network_params"])
        network_folder = utils.convert_data_relative_path_to_absolute_path(
            network_folder, assert_path_exists=True)
        dataset_config = utils.getDictFromYamlFilename(
            os.path.join(network_folder, "dataset.yaml"))
    ...
    def get_image_pair_with_poses_diff_above_threshold(self, dataset, scene_name, threshold=0.05,
        ...
        pose_a = dataset.get_pose_from_scene_name_and_idx(
            scene_name, img_a_idx)
        pos_a = pose_a[0:3, 3]

        for i in range(0, max_num_attempts):
            img_b_idx = dataset.get_random_image_index(scene_name)
            pose_b = dataset.get_pose_from_scene_name_and_idx(
    ...
    def evaluate_single_network(self, network_name, mode="train", save=True):
        ...
        pd_dataframe_list, df = self.evaluate_network(dcn, dataset, num_image_pairs=num_image_pairs,
                                                      num_matches_per_image_pair=num_matches_per_image_pair)

        # save pandas.DataFrame to csv
        if save:
            output_dir = os.path.join(
                self.get_output_dir(), network_name, mode)
    ...
    def evaluate_single_network_cross_scene(self, network_name, save=True):
        ...
        self.evaluate_network_cross_scene(
            dcn, dataset, save=save)

    def evaluate_network_cross_scene(self, dcn=None, dataset=None, save=True):
        """
        ...
        cross_scene_data = self.parse_cross_scene_data(
            dataset)
        if cross_scene_data is None:
            print("No Cross Scene Data!")
            return
        ...
        for annotated_pair in cross_scene_data:
            ...
            if not os.path.isdir(dataset.get_full_path_for_scene(scene_name_a))\
                ...
                print("at least one of these scene names does not exist:",
                      scene_name_a, scene_name_b)
                continue
            ...
            dataframe_list_temp =\
                self.single_cross_scene_image_pair_quantitative_analysis(dcn,
        ...
        if save:
            output_dir = os.path.join(
                self.get_output_dir(), network_name, "cross-scene")
            data_file = os.path.join(output_dir, "data.csv")
            if not os.path.isdir(output_dir):
                os.makedirs(output_dir)
    ...
    def evaluate_network_across_objects(self, dcn=None, dataset=None, num_image_pairs=25):
        """
        ...
        for i in range(num_image_pairs):
            ...
            scene_name_a = dataset.get_random_single_object_scene_name(
                object_id_a)
            scene_name_b = dataset.get_random_single_object_scene_name(
                object_id_b)
            ...
            dataframe_list_temp =\
                self.single_across_object_image_pair_quantitative_analysis(dcn,
    ...
    def evaluate_single_network_cross_instance(self, network_name, full_path_cross_instance_labels, save=False):
        ...
        dcn.eval()
        dataset = dcn.load_training_dataset()
        return self.evaluate_network_cross_instance(dcn, dataset, full_path_cross_instance_labels, save=False)

    def evaluate_network_cross_instance(self, dcn=None, dataset=None, full_path_cross_instance_labels=None, save=False):
        """
        ...
        cross_instance_keypoint_labels = utils.getDictFromYamlFilename(
            full_path_cross_instance_labels)
        ...
        if save:
            output_dir = os.path.join(
                self.get_output_dir(), network_name, "cross-instance")
            data_file = os.path.join(output_dir, "data.csv")
            if not os.path.isdir(output_dir):
                os.makedirs(output_dir)
    ...
    def evaluate_network_cross_scene_keypoints(self, dcn, dataset, full_path_cross_instance_labels):
        """
        ...
        for keypoint_label in cross_instance_keypoint_labels:
            ...
            res = dcn.forward_single_image_tensor(
                rgb_tensor).data.cpu().numpy()
        ...
        for subset in itertools.combinations(cross_instance_keypoint_labels, 2):
            ...
            res_a = descriptor_images[keypoint_data_a["scene_name"]
                                      ][keypoint_data_a["image_idx"]]
            res_b = descriptor_images[keypoint_data_b["scene_name"]
                                      ][keypoint_data_b["image_idx"]]

            dataframe_list_temp = \
                self.single_image_pair_cross_scene_keypoints_quantitative_analysis(
                    dcn, dataset, keypoint_data_a, keypoint_data_b, res_a, res_b)
    ...
    def evaluate_network(self, dcn, dataset, num_image_pairs=25, num_matches_per_image_pair=100):
        """
        ...
        for i in range(0, num_image_pairs):
            ...
            # grab random scene
            if i % logging_rate == 0:
                print("computing statistics for image %d of %d, scene_name %s" % (
                    i, num_image_pairs, scene_name))
                print("scene")

            idx_pair = self.get_image_pair_with_poses_diff_above_threshold(
                dataset, scene_name)
            ...
            dataframe_list_temp = self.single_same_scene_image_pair_quantitative_analysis(dcn, dataset, scene_name,
                                                                                          img_idx_a,
                                                                                          img_idx_b,
    ...
    def plot_descriptor_colormaps(self, res_a, res_b, descriptor_image_stats=None,
        ...
        if descriptor_image_stats is None:
            res_a_norm, res_b_norm = dc_plotting.normalize_descriptor_pair(
                res_a, res_b)
        else:
            res_a_norm = dc_plotting.normalize_descriptor(
                res_a, descriptor_image_stats[descriptor_norm_type])
            res_b_norm = dc_plotting.normalize_descriptor(
                res_b, descriptor_image_stats[descriptor_norm_type])
        ...
        if plot_masked:
            ...
            if descriptor_image_stats is None:
                res_a_norm_mask, res_b_norm_mask = dc_plotting.normalize_masked_descriptor_pair(
                    res_a, res_b, mask_a, mask_b)
            else:
                res_a_norm_mask = dc_plotting.normalize_descriptor(
                    res_a_mask, descriptor_image_stats['mask_image'])
                res_b_norm_mask = dc_plotting.normalize_descriptor(
                    res_b_mask, descriptor_image_stats['mask_image'])
    ...
    def clip_pixel_to_image_size_and_round(self, uv, image_width, image_height):
        u = min(int(round(uv[0])), image_width - 1)
        v = min(int(round(uv[1])), image_height - 1)
        return (u, v)

    def single_cross_scene_image_pair_quantitative_analysis(self, dcn, dataset, scene_name_a,
        ...
        rgb_a, depth_a, mask_a, pose_a = dataset.get_rgbd_mask_pose(
            scene_name_a, img_a_idx)

        rgb_b, depth_b, mask_b, pose_b = dataset.get_rgbd_mask_pose(
            scene_name_b, img_b_idx)
        ...
        res_a = dcn.forward_single_image_tensor(
            rgb_a_tensor).data.cpu().numpy()
        res_b = dcn.forward_single_image_tensor(
            rgb_b_tensor).data.cpu().numpy()
        ...
        if not np.allclose(camera_intrinsics_a.K, camera_intrinsics_b.K):
            print(
                "Currently cannot handle two different camera K matrices in different scenes!")
        ...
        for i in range(len(img_a_pixels)):
            ...
            for j in range(J):
                different_view_a_idx = dataset.get_img_idx_with_different_pose(
                    scene_name_a, pose_a, num_attempts=50)
                if different_view_a_idx is None:
                    logging.info(
                        "no frame with sufficiently different pose found, continuing")
                    continue
                diff_rgb_a, diff_depth_a, diff_mask_a, diff_pose_a = dataset.get_rgbd_mask_pose(
                    scene_name_a, different_view_a_idx)
                ...
                diff_res_a = dcn.forward_single_image_tensor(
                    diff_rgb_a_tensor).data.cpu().numpy()

                diff_uv_a = (diff_uv_a_vec[0][0], diff_uv_a_vec[1][0])
                diff_uv_a = self.clip_pixel_to_image_size_and_round(
                    diff_uv_a, image_width, image_height)

                pd_template = self.compute_descriptor_match_statistics(diff_depth_a,
            ...
            for k in range(K):
                different_view_b_idx = dataset.get_img_idx_with_different_pose(
                    scene_name_b, pose_b, num_attempts=50)
                if different_view_b_idx is None:
                    logging.info(
                        "no frame with sufficiently different pose found, continuing")
                    continue
                diff_rgb_b, diff_depth_b, diff_mask_b, diff_pose_b = dataset.get_rgbd_mask_pose(
                    scene_name_b, different_view_b_idx)
                ...
                diff_res_b = dcn.forward_single_image_tensor(
                    diff_rgb_b_tensor).data.cpu().numpy()

                diff_uv_b = (diff_uv_b_vec[0][0], diff_uv_b_vec[1][0])
                diff_uv_b = self.clip_pixel_to_image_size_and_round(
                    diff_uv_b, image_width, image_height)
    ...
    def single_across_object_image_pair_quantitative_analysis(self, dcn, dataset, scene_name_a, scene_name_b,
        ...
        rgb_a, depth_a, mask_a, _ = dataset.get_rgbd_mask_pose(
            scene_name_a, img_a_idx)
        rgb_b, depth_b, mask_b, _ = dataset.get_rgbd_mask_pose(
            scene_name_b, img_b_idx)
        ...
        sampled_idx_list = random_sample_from_masked_image(
            mask_a, num_uv_a_samples)
        ...
        for i in range(num_uv_a_samples):
            ...
            pd_template = self.compute_descriptor_match_statistics_no_ground_truth(uv_a, res_a,
                                                                                   res_b,
                                                                                   rgb_a=rgb_a,
                                                                                   rgb_b=rgb_b,
                                                                                   depth_a=depth_a,
    ...
    def single_same_scene_image_pair_quantitative_analysis(self, dcn, dataset, scene_name,
                                                           img_a_idx, img_b_idx,
        ...
        for i in match_list:
            ...
            uv_b = self.clip_pixel_to_image_size_and_round(
                uv_b_raw, image_width, image_height)
    ...
    def compute_descriptor_match_statistics_no_ground_truth(self, uv_a, res_a, res_b, rgb_a=None, rgb_b=None,
        ...
        # compute best match
        uv_b, best_match_diff, norm_diffs = self.Network.find_best_match(uv_a, res_a,
                                                                         res_b, debug=debug)
        ...
        pd_template.set_value(
            'norm_diff_descriptor_best_match', best_match_diff)
        return pd_template

    def compute_descriptor_match_statistics(self, depth_a, depth_b, mask_a, mask_b, uv_a, uv_b, pose_a, pose_b,
        ...
        # compute best match
        uv_b_pred, best_match_diff, norm_diffs = self.Network.find_best_match(uv_a, res_a,
                                                                              res_b, debug=debug)
        ...
        best_match_xy_masked = np.unravel_index(
            best_match_flattened_idx_masked, masked_norm_diffs.shape)
        ...
        pixel_match_error_l2 = np.linalg.norm(
            (np.array(uv_b) - np.array(uv_b_pred)), ord=2)
        pixel_match_error_l2_masked = np.linalg.norm(
            (np.array(uv_b) - np.array(uv_b_pred_masked)), ord=2)
        pixel_match_error_l1 = np.linalg.norm(
            (np.array(uv_b) - np.array(uv_b_pred)), ord=1)

        # extract the ground truth descriptors
        des_a = res_a[uv_a[1], uv_a[0], :]
        des_b_ground_truth = res_b[uv_b[1], uv_b[0], :]
        norm_diff_descriptor_ground_truth = np.linalg.norm(
            des_a - des_b_ground_truth)
        ...
        num_pixels_closer_than_ground_truth = len(
            u_indices_better_than_ground_truth)
        num_pixels_in_image = res_a.shape[0] * res_a.shape[1]
        fraction_pixels_closer_than_ground_truth = num_pixels_closer_than_ground_truth * \
            1.0 / num_pixels_in_image
        ...
        num_pixels_closer_than_ground_truth_masked = len(
            u_indices_better_than_ground_truth_masked)
        num_pixels_in_masked_image = len(np.nonzero(mask_b)[0])
        fraction_pixels_closer_than_ground_truth_masked = num_pixels_closer_than_ground_truth_masked * \
            1.0 / num_pixels_in_masked_image
        ...
        if num_pixels_closer_than_ground_truth_masked == 0:
            ...
            average_l2_distance_for_false_positives_masked = np.average(
                l2_distances_masked)

        # extract depth values, note the indexing order of u,v has to be reversed
        uv_a_depth = depth_a[uv_a[1], uv_a[0]] / \
            DEPTH_IM_SCALE  # check if this is not None
        uv_b_depth = depth_b[uv_b[1], uv_b[0]] / DEPTH_IM_SCALE
        uv_b_pred_depth = depth_b[uv_b_pred[1], uv_b_pred[0]] / DEPTH_IM_SCALE
        uv_b_pred_depth_is_valid = self.is_depth_valid(
            uv_b_pred_depth)
        uv_b_pred_depth_masked = depth_b[uv_b_pred_masked[1],
                                         uv_b_pred_masked[0]] / DEPTH_IM_SCALE
        uv_b_pred_depth_is_valid_masked = self.is_depth_valid(
            uv_b_pred_depth_masked)
        is_valid = uv_b_pred_depth_is_valid
        is_valid_masked = uv_b_pred_depth_is_valid_masked

        uv_a_pos = self.compute_3d_position(
            uv_a, uv_a_depth, camera_matrix, pose_a)
        uv_b_pos = self.compute_3d_position(
            uv_b, uv_b_depth, camera_matrix, pose_b)
        uv_b_pred_pos = self.compute_3d_position(
            uv_b_pred, uv_b_pred_depth, camera_matrix, pose_b)
        uv_b_pred_pos_masked = self.compute_3d_position(
            uv_b_pred_masked, uv_b_pred_depth_masked, camera_matrix, pose_b)
        ...
        if self.is_depth_valid(uv_b_depth):
            norm_diff_ground_truth_3d = np.linalg.norm(diff_ground_truth_3d)
        else:
            norm_diff_ground_truth_3d = np.nan

        if self.is_depth_valid(uv_b_depth) and self.is_depth_valid(uv_b_pred_depth):
            norm_diff_pred_3d = np.linalg.norm(diff_pred_3d)
        else:
            norm_diff_pred_3d = np.nan

        if self.is_depth_valid(uv_b_depth) and self.is_depth_valid(uv_b_pred_depth_masked):
        ...
        pd_template.set_value(
            'norm_diff_descriptor_masked', best_match_diff_masked)
        pd_template.set_value('is_valid', is_valid)
        pd_template.set_value('is_valid_masked', is_valid_masked)

        pd_template.set_value('norm_diff_ground_truth_3d',
                              norm_diff_ground_truth_3d)
        ...
        if is_valid_masked:
            pd_template.set_value(
                'norm_diff_pred_3d_masked', norm_diff_pred_3d_masked)
        else:
            pd_template.set_value('norm_diff_pred_3d_masked', np.nan)

        pd_template.set_value(
            'norm_diff_descriptor_ground_truth', norm_diff_descriptor_ground_truth)

        pd_template.set_value('pixel_match_error_l2', pixel_match_error_l2)
        pd_template.set_value('pixel_match_error_l2_masked',
                              pixel_match_error_l2_masked)
        pd_template.set_value('pixel_match_error_l1', pixel_match_error_l1)

        pd_template.set_value('fraction_pixels_closer_than_ground_truth',
                              fraction_pixels_closer_than_ground_truth)
        pd_template.set_value('fraction_pixels_closer_than_ground_truth_masked',
                              fraction_pixels_closer_than_ground_truth_masked)
        pd_template.set_value(
            'average_l2_distance_for_false_positives', average_l2_distance_for_false_positives)
        pd_template.set_value('average_l2_distance_for_false_positives_masked',
                              average_l2_distance_for_false_positives_masked)

        return pd_template

    def compute_3d_position(self, uv, depth, camera_intrinsics_matrix, camera_to_world):
        """
        ...
        """
        pos_in_camera_frame = correspondence_finder.pinhole_projection_image_to_world(
            uv, depth, camera_intrinsics_matrix)

        pos_in_world_frame = np.dot(
            camera_to_world, np.append(pos_in_camera_frame, 1))[:3]

        return pos_in_world_frame

    def single_same_scene_image_pair_qualitative_analysis(self, dcn, dataset, scene_name,
                                                          img_a_idx, img_b_idx,
                                                          num_matches=10, save_path=None):
        """
        ...
        self.single_image_pair_qualitative_analysis(
            dcn, dataset, rgb_a, rgb_b, mask_a, mask_b, num_matches, save_path=save_path)

    def single_cross_scene_image_pair_qualitative_analysis(self, dcn, dataset, scene_name_a,
                                                           img_a_idx, scene_name_b, img_b_idx,
                                                           num_matches=10):
        """
        ...
        rgb_a, _, mask_a, _ = dataset.get_rgbd_mask_pose(
            scene_name_a, img_a_idx)

        rgb_b, _, mask_b, _ = dataset.get_rgbd_mask_pose(
            scene_name_b, img_b_idx)
    ...
    def single_image_pair_keypoint_qualitative_analysis(self, dcn, dataset, keypoint_data_a,
                                                        keypoint_data_b,
        ...
        best_match_uv, best_match_diff, norm_diffs = self.Network.find_best_match(
    ...
    def single_image_pair_qualitative_analysis(self, dcn, dataset, rgb_a, rgb_b, mask_a, mask_b,
                                               num_matches, save_path=None):
        """
        ...
        for i in range(0, num_matches):
            ...
            best_match_uv, best_match_diff, norm_diffs = self.Network.find_best_match(pixel_a, res_a,
                                                                                      res_b)
        ...
        img3 = cv2.drawMatches(
            gray_a_numpy, kp1, gray_b_numpy, kp2, matches, flags=2, outImg=gray_b_numpy)
        fig, axes = plt.subplots(nrows=1, ncols=1)
        fig.set_figheight(10)
        fig.set_figwidth(15)
        axes.imshow(img3)
        if save_path is not None:
            plt.savefig(save_path + "_matches.png")
        # show colormap if possible (i.e. if descriptor dimension is 1 or 3)
        assert dcn.descriptor_dim == 3
        if dcn.descriptor_dimension in [1, 3]:
            self.plot_descriptor_colormaps(res_a, res_b,
        ...
        if save_path is not None:
            plt.savefig(save_path + "_descriptors.png")
        plt.show()

    def single_image_pair_cross_scene_keypoints_quantitative_analysis(self, dcn, dataset, keypoint_data_a,
        ...
        for kp_name, data_a in keypoint_data_a['keypoints'].items():
            ...
            for order in ordering:
                ...
                uv_1 = self.clip_pixel_to_image_size_and_round(
                    (data[idx_1]['u'], data[idx_2]['v']), image_width, image_height)
                uv_2 = self.clip_pixel_to_image_size_and_round((data[idx_2]['u'], data[idx_2]['v']), image_width,
                                                               image_height)

                pd_template = self.compute_descriptor_match_statistics(depth[idx_1],
                                                                       depth[idx_2],
                                                                       mask[idx_1],
                                                                       mask[idx_2],
                                                                       uv_1,
    ...
    def compute_sift_keypoints(self, img, mask=None):
        """
    ...
    def single_image_pair_sift_analysis(self, dataset, scene_name,
                                        img_a_idx, img_b_idx,
        ...
        kp1, des1, gray1, img_1_kp = self.compute_sift_keypoints(rgb_a, mask_a)
        kp2, des2, gray2, img_2_kp = self.compute_sift_keypoints(rgb_b, mask_b)
        ...
        if visualize:
            ...
            img3 = cv2.drawMatchesKnn(
                img1, kp1, img2, kp2, good_vis, outImg, flags=2)
            plt.imshow(img3)
        ...
        for idx, val in enumerate(good):
            ...
            df = self.compute_single_sift_match_statistics(depth_a, depth_b, kp_a, kp_b,
    ...
    def compute_single_sift_match_statistics(self, depth_a, depth_b, kp_a, kp_b, pose_a, pose_b,
        ...
        # print("pose_a", pose_a)
        kp_a_3d = self.compute_3d_position(
            uv_a, uv_a_depth, camera_matrix, pose_a)

        uv_b = clip_pixel_to_image_size_and_round((kp_b.pt[0], kp_b.pt[1]))
        uv_b_depth = depth_b[uv_b[1], uv_b[0]] / DEPTH_IM_SCALE
        uv_b_depth_valid = self.is_depth_valid(uv_b_depth)
        kp_b_3d = self.compute_3d_position(
            uv_b, uv_b_depth, camera_matrix, pose_b)
    ...
    def single_image_pair_keypoint_analysis(self, dcn, dataset, scene_name,
                                            img_a_idx, img_b_idx,
                                            params=None,
                                            camera_intrinsics_matrix=None, visualize=True):

        # first compute SIFT stuff
        sift_data = self.single_image_pair_sift_analysis(dataset, scene_name,
    ...
    def parse_cross_scene_data(self, dataset):
        """
        ...
        for path in evaluation_labeled_data_paths:
            cross_scene_data_full_path = utils.convert_data_relative_path_to_absolute_path(
                path, assert_path_exists=True)
            this_cross_scene_data = utils.getDictFromYamlFilename(
                cross_scene_data_full_path)
            cross_scene_data += this_cross_scene_data

        return cross_scene_data

    def evaluate_network_qualitative_cross_scene(self, dcn, dataset, draw_human_annotations=True):
        """
        This will search for the "evaluation_labeled_data_path" in the dataset.yaml,
        and use pairs of images that have been human-labeled across scenes.
        """
        ...
        for annotated_pair in cross_scene_data:
            ...
            rgb_a, rgb_b = self.single_cross_scene_image_pair_qualitative_analysis(
            ...
            if draw_human_annotations:
                ...
                rgb_a = dc_plotting.draw_correspondence_points_cv2(
                    np.asarray(rgb_a), img_a_points_picked)
                rgb_b = dc_plotting.draw_correspondence_points_cv2(
                    np.asarray(rgb_b), img_b_points_picked)
    ...
    def get_random_image_pairs(self, dataset, scene_name=None, num_image_pairs=5):
        """
        ...
        for _ in range(num_image_pairs):
            ...
            pose_a = dataset.get_pose_from_scene_name_and_idx(
                scene_name, img_a_idx)
            img_b_idx = dataset.get_img_idx_with_different_pose(
                scene_name, pose_a, num_attempts=100)
            if img_b_idx is None:
                continue
            img_pairs.append([img_a_idx, img_b_idx])
        return scene_name, img_pairs

    def get_random_scenes_and_image_pairs(self, dataset, object_id=None, num_image_pairs=5):
        """
        ...
        img_pairs = []
        for _ in range(num_image_pairs):
            scene_name = dataset.get_random_scene_name(object_id=object_id)
            img_a_idx = dataset.get_random_image_index(scene_name)
            pose_a = dataset.get_pose_from_scene_name_and_idx(
                scene_name, img_a_idx)
            img_b_idx = dataset.get_img_idx_with_different_pose(
    ...
    def evaluate_network_qualitative(self, dcn, dataset, num_image_pairs=5, randomize=False,
                                     object_id=None, save_path=None):

        dcn.eval()
        # # Train Data
        # print("\n\n-----------Train Data Evaluation----------------")
        # if randomize:
        #     scene_names, img_pairs = self.get_random_scenes_and_image_pairs(
        #         dataset)
        # else:
        #     if object_id == "caterpillar":
        ...
        #     else:
        #         raise ValueError(
        #             "object_id must be one of [drill, caterpillar], it was %s)" % (object_id))
        ...
        #     try:
        #         self.single_same_scene_image_pair_qualitative_analysis(dcn,
        #                                                                dataset,
        #                                                                scene_name,
        #                                                                img_pair[0],
        #                                                                img_pair[1])
        #     except Exception as e:
        #         print("Failed to visualize due to: %s" % e)

        # Test Data
        print("\n\n-----------Test Data Evaluation on %s----------------" % object_id)
        dataset.set_test_mode()
        if randomize:
            scene_names, img_pairs = self.get_random_scenes_and_image_pairs(
                dataset, object_id=object_id, num_image_pairs=num_image_pairs)
        else:
            if object_id == "caterpillar":
                ...
                raise ValueError(
                    "Non-randomized object_id must be one of [drill, caterpillar], it was %s)" % (object_id))
        ...
        for scene_name, img_pair in zip(scene_names, img_pairs):
            ...
            try:
                self.single_same_scene_image_pair_qualitative_analysis(dcn,
                                                                       ...
                                                                       scene_name,
                                                                       img_pair[0],
                                                                       img_pair[1], save_path=save_path)
            except Exception as e:
                print("Failed to visualize due to: %s" % e)

    def compute_loss_on_dataset(self, dcn, data_loader, calc_losses_fn, num_iterations=500,):
        """
        ...
        total_masked_non_match_loss = 0.0
        total_background_non_match_loss = 0.0
        total_blind_non_match_loss = 0.0
        batch_size = 1
        count = 0
        for i, sample in enumerate(data_loader, 0):
            ...
            with torch.no_grad():
                (loss, match_loss, masked_non_match_loss,
                    background_non_match_loss, blind_non_match_loss) = calc_losses_fn(dcn, sample)

                if np.isclose(loss.item(), 0.0, atol=1e-8):
                    continue

                count += 1
                total_loss += loss.flatten().cpu()
                total_match_loss += match_loss.flatten().cpu()
                total_masked_non_match_loss += masked_non_match_loss.flatten().cpu()
                total_background_non_match_loss += background_non_match_loss.flatten().cpu()
                total_blind_non_match_loss += blind_non_match_loss.flatten().cpu()

        return total_loss / count, total_match_loss / count, total_masked_non_match_loss / count, total_background_non_match_loss / count, total_blind_non_match_loss / count

    def compute_descriptor_statistics_on_dataset(self, dcn, dataset, num_images=100,
        ...
        def compute_descriptor_statistics(self, res, mask_tensor):
            """
            Computes
            ...
            res_masked_flat = res_reshape.index_select(
                0, mask_indices_flat)  # shape [mask_size, D]
            ...
            mask_image_stats = (
                mask_channel_min, mask_channel_max, mask_channel_mean)
            return entire_image_stats, mask_image_stats

        def compute_descriptor_std_dev(self, res, channel_mean):
            """
        ...
        def update_stats(self, stats_dict, single_img_stats):
            """
        ...
        for i in range(0, num_images):
            ...
            entire_image_stats, mask_image_stats = compute_descriptor_statistics(
                res, mask_tensor)
        ...
        if save_to_file:
            if filename is None:
                ...
                path_to_params_folder = utils.convert_data_relative_path_to_absolute_path(
                    path_to_params_folder)
                filename = os.path.join(
                    path_to_params_folder, 'descriptor_statistics.yaml')

            utils.saveToYaml(stats, filename)

        return stats

    def run_evaluation_on_network(self, model_folder, num_image_pairs=100,
        ...
        If dataset not specified, evaluates on the dataset used for training
        Datasets have "train" and "test" modes
        ...
        model_folder = utils.convert_data_relative_path_to_absolute_path(
            model_folder, assert_path_exists=True)
        ...
        if compute_descriptor_statistics:
            ...
            self.compute_descriptor_statistics_on_dataset(
                dcn, dataset, num_images=100, save_to_file=True)
        ...
        if cross_scene:
            ...
            df = self.evaluate_network_cross_scene(
                dcn=dcn, dataset=dataset, save=False)
            if df is not None:
                cross_scene_csv = os.path.join(
                    cross_scene_output_dir, "data.csv")
                df.to_csv(cross_scene_csv)
            else:
                cross_scene_csv = None

        logging.info("Making plots")
        fig_axes = self.run_on_single_dataframe(
            train_csv, label="train", save=False)
        fig_axes = self.run_on_single_dataframe(
            test_csv, label="test", save=False, previous_fig_axes=fig_axes)
        if cross_scene and cross_scene_csv is not None:
            fig_axes = self.run_on_single_dataframe(cross_scene_csv, label="cross_scene", save=False,
        ...
        if dataset.get_number_of_unique_single_objects() > 1:
            across_object_output_dir = os.path.join(
                output_dir, "across_object")
            ...
            df = self.evaluate_network_across_objects(dcn=dcn, dataset=dataset)
            across_object_csv = os.path.join(
                across_object_output_dir, "data.csv")
            df.to_csv(across_object_csv)
            self.run_on_single_dataframe_across_objects(
                across_object_csv, label="across_object", save=True)

        logging.info("Finished running evaluation on network")

    def run_cross_instance_keypoint_evaluation_on_network(self, model_folder, path_to_cross_instance_labels,
        ...
        dcn = self.Network.from_model_folder(model_folder)
        dcn.eval()

        if dataset is None:
            dataset = dcn.load_training_dataset()

        path_to_cross_instance_labels = utils.convert_data_relative_path_to_absolute_path(
            path_to_cross_instance_labels, assert_path_exists=True)
        df = self.evaluate_network_cross_scene_keypoints(
            dcn, dataset, path_to_cross_instance_labels)
    ...
    def make_2d_cluster_plot(self, dcn, dataset, plot_background=False):
        """
        ...
        assert ((dcn.descriptor_dimension == 2)
                or (dcn.descriptor_dimension == 3))
        ...
        for i in range(100):
            ...
            rgb = dataset.get_rgb_image_from_scene_name_and_idx(
                scene_name, img_idx)
            mask = dataset.get_mask_image_from_scene_name_and_idx(
                scene_name, img_idx)

            mask_torch = torch.from_numpy(np.asarray(mask)).long()
            mask_inv = 1 - mask_torch

            object_uv_samples = correspondence_finder.random_sample_from_masked_image_torch(
                mask_torch, num_samples_per_image)
            background_uv_samples = correspondence_finder.random_sample_from_masked_image_torch(
                mask_inv, num_samples_per_image / num_objects)
            ...
            for j in range(len(object_u_samples)):
                descriptors_object[j, :] = res[object_v_samples[j],
                                               object_u_samples[j], :]
            if use_3d:
                ...
                for j in range(len(object_u_samples)):
                    descriptors_object_xy[j, :] = res[object_v_samples[j],
                                                      object_u_samples[j], 0:2]
                    descriptors_object_yz[j, :] = res[object_v_samples[j],
                                                      object_u_samples[j], 1:3]
                    descriptors_object_xz[j, :] = res[object_v_samples[j],
                                                      object_u_samples[j], 0::2]

            descriptors_background = np.zeros((len(background_u_samples), d))
            for j in range(len(background_u_samples)):
                descriptors_background[j, :] = res[background_v_samples[j],
                                                   background_u_samples[j], :]
            if use_3d:
                descriptors_background_xy = np.zeros(
                    (len(background_u_samples), 2))
                descriptors_background_yz = np.zeros(
                    (len(background_u_samples), 2))
                descriptors_background_xz = np.zeros(
                    (len(background_u_samples), 2))
                for j in range(len(background_u_samples)):
                    descriptors_background_xy[j, :] = res[background_v_samples[j],
                                                          background_u_samples[j], 0:2]
                    descriptors_background_yz[j, :] = res[background_v_samples[j],
                                                          background_u_samples[j], 1:3]
                    descriptors_background_xz[j, :] = res[background_v_samples[j],
                                                          background_u_samples[j], 0::2]
            ...
            if object_id not in descriptors_known_objects_samples:
                ...
                descriptors_known_objects_samples[object_id] = np.vstack(
                    (descriptors_known_objects_samples[object_id], descriptors_object))
                if use_3d:
                    descriptors_known_objects_samples_xy[object_id] = np.vstack(
                        (descriptors_known_objects_samples_xy[object_id], descriptors_object_xy))
                    descriptors_known_objects_samples_yz[object_id] = np.vstack(
                        (descriptors_known_objects_samples_yz[object_id], descriptors_object_yz))
                    descriptors_known_objects_samples_xz[object_id] = np.vstack(
                        (descriptors_known_objects_samples_xz[object_id], descriptors_object_xz))

            descriptors_background_samples = np.vstack(
                (descriptors_background_samples, descriptors_background))
            if use_3d:
                descriptors_background_samples_xy = np.vstack(
                    (descriptors_background_samples_xy, descriptors_background_xy))
                descriptors_background_samples_yz = np.vstack(
                    (descriptors_background_samples_yz, descriptors_background_yz))
                descriptors_background_samples_xz = np.vstack(
                    (descriptors_background_samples_xz, descriptors_background_xz))
        ...
        if not use_3d:
            ...
            if plot_background:
                plt.scatter(
                    descriptors_background_samples[:, 0], descriptors_background_samples[:, 1], alpha=0.5, label="background")
            plt.legend()
            plt.show()
        ...
        if use_3d:
            ...
            if plot_background:
                plt.scatter(
                    descriptors_background_samples_xy[:, 0], descriptors_background_samples_xy[:, 1], alpha=0.5, label="background")
            plt.legend()
            plt.show()
            ...
            if plot_background:
                plt.scatter(
                    descriptors_background_samples_yz[:, 0], descriptors_background_samples_yz[:, 1], alpha=0.5, label="background")
            plt.legend()
            plt.show()
            ...
            if plot_background:
                plt.scatter(
                    descriptors_background_samples_xz[:, 0], descriptors_background_samples_xz[:, 1], alpha=0.5, label="background")
            plt.legend()
            plt.show()
    ...
    def test(self, dcn, dataset, data_idx=1, visualize=False, debug=False, match_idx=10):

        scene_name = '13_drill_long_downsampled'
        img_idx_a = utils.getPaddedString(0)
        img_idx_b = utils.getPaddedString(737)

        self.single_same_scene_image_pair_qualitative_analysis(dcn, dataset,
...
class DenseCorrespondenceEvaluationPlotter(object):
    ...
    def make_cdf_plot(self, ax, data, num_bins, label=None, x_axis_scale_factor=1):
        """
    ...
    def make_pixel_match_error_plot(self, ax, df, label=None, num_bins=100, masked=False):
        """
        :param ax: axis of a matplotlib plot to plot on
        ...
        plot = self.make_cdf_plot(
            ax, data, num_bins=num_bins, label=label, x_axis_scale_factor=x_axis_scale_factor)
        if masked:
            ax.set_xlabel('Pixel match error (masked), L2 (pixel distance)')
        else:
            ax.set_xlabel(
                'Pixel match error (fraction of image), L2 (pixel distance)')
        ax.set_ylabel('Fraction of images')

        # ax.set_xlim([0,200])
        return plot

    def make_across_object_best_match_plot(self, ax, df, label=None, num_bins=100):
        """
        :param ax: axis of a matplotlib plot to plot on
        :param df: pandas dataframe, i.e. generated from quantitative
        :param num_bins:
        ...
        plot = self.make_cdf_plot(ax, data, num_bins=num_bins, label=label)
        ax.set_xlabel('Best descriptor match, L2 norm')
        ax.set_ylabel('Fraction of pixel samples from images')
        return plot

    def make_descriptor_accuracy_plot(self, ax, df, label=None, num_bins=100, masked=False):
        """
    ...
    def make_norm_diff_ground_truth_plot(self, ax, df, label=None, num_bins=100, masked=False):
        """
        :param ax: axis of a matplotlib plot to plot on
        :param df:
        :type df:
    ...
    def make_fraction_false_positives_plot(self, ax, df, label=None, num_bins=100, masked=False):
        """
        :param ax: axis of a matplotlib plot to plot on
        :param df:
        :type df:
    ...
    def make_average_l2_false_positives_plot(self, ax, df, label=None, num_bins=100, masked=False):
        """
        :param ax: axis of a matplotlib plot to plot on
        :param df:
        :type df:
        ...
        if masked:
            ax.set_xlabel(
                'Average l2 pixel distance for false positives (masked)')
        else:
    ...
    def compute_area_above_curve(self, df, field, num_bins=100):
        """
    ...
    def run_on_single_dataframe(self, path_to_df_csv, label=None, output_dir=None, save=True, previous_fig_axes=None, dataframe=None):
        """
        ...
        - if you'd like each line to have a label in the plot, then use pass a string to label, i.e.:
            ...
            run_on_single_dataframe(
                "thing2.csv", label="thing2", previous_plot=fig)
            plt.title("both things")
            plt.show()
        ...
        # pixel match error
        ax = get_ax(axes, 0)
        plot = self.make_pixel_match_error_plot(ax, df, label=label)
        if use_masked_plots:
            plot = self.make_pixel_match_error_plot(
                axes[0, 1], df, label=label, masked=True)
        ax.legend()

        # 3D match error
        ax = get_ax(axes, 1)
        plot = self.make_descriptor_accuracy_plot(ax, df, label=label)
        if use_masked_plots:
            plot = self.make_descriptor_accuracy_plot(
                axes[1, 1], df, label=label, masked=True)

        # if save:
        #     fig_file = os.path.join(output_dir, "norm_diff_pred_3d.png")
        #     fig.savefig(fig_file)

        aac = self.compute_area_above_curve(df, 'norm_diff_pred_3d')
        d = dict()
        d['norm_diff_3d_area_above_curve'] = float(aac)

        # norm difference of the ground truth match (should be 0)
        ax = get_ax(axes, 2)
        plot = self.make_norm_diff_ground_truth_plot(ax, df, label=label)

        # fraction false positives
        ax = get_ax(axes, 3)
        plot = self.make_fraction_false_positives_plot(ax, df, label=label)
        if use_masked_plots:
            plot = self.make_fraction_false_positives_plot(
                axes[3, 1], df, label=label, masked=True)

        # average l2 false positives
        ax = get_ax(axes, 4)
        plot = self.make_average_l2_false_positives_plot(ax, df, label=label)
        if use_masked_plots:
            plot = self.make_average_l2_false_positives_plot(
                axes[4, 1], df, label=label, masked=True)
    ...
    def run_on_single_dataframe_across_objects(self, path_to_df_csv, label=None, output_dir=None, save=True, previous_fig_axes=None):
        """
        This method is intended to be called from an ipython notebook for plotting.
        ...
        # pixel match error
        plot = self.make_across_object_best_match_plot(ax, df, label=label)
        ax.legend()
...
class OPMultiDenseCorrespondenceEvaluation(DenseCorrespondenceEvaluation):
    Network = OPMultiDenseCorrespondenceNetwork
    ...
    def single_same_scene_image_pair_qualitative_analysis(self, dcn, dataset, scene_name,
        ...
        :return: None
        """
        object_id, object_id_int = dataset.get_object_id_and_int_from_scene_name(
            scene_name)
        rgb_a, _, mask_a, _ = dataset.get_rgbd_mask_pose(scene_name, img_a_idx)
        ...
        self.single_image_pair_qualitative_analysis(
            dcn, dataset, rgb_a, rgb_b, mask_a, mask_b, num_matches, object_id_int, save_path=save_path)

    def single_image_pair_qualitative_analysis(self, dcn, dataset, rgb_a, rgb_b, mask_a, mask_b,
                                               num_matches, object_id_int, save_path):
        """
        Computes qualtitative assessment of DCN performance for a pair of
        images
        ...
        # these are Variables holding torch.FloatTensors, first grab the data, then convert to numpy
        res_a = dcn.forward_single_image_tensor(
            rgb_a_tensor, object_id_int).data.cpu().numpy()
        res_b = dcn.forward_single_image_tensor(
            rgb_b_tensor, object_id_int).data.cpu().numpy()
