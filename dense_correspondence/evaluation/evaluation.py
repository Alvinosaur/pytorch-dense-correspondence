...
class DenseCorrespondenceEvaluation(object):
    ...
    def evaluate_network(self, dcn, dataset, num_image_pairs=25, num_matches_per_image_pair=100):
        ...
        for i in xrange(0, num_image_pairs):
            ...
            dataframe_list_temp =\
                self.single_same_scene_image_pair_quantitative_analysis(dcn, dataset, scene_name,
                                                                        img_idx_a,
                                                                        img_idx_b,
    ...
    def compute_descriptor_match_statistics_no_ground_truth(self, uv_a, res_a, res_b, rgb_a=None, rgb_b=None,
        ...
        uv_b, best_match_diff, norm_diffs =\
            self.Network.find_best_match(uv_a, res_a,
                                         res_b, debug=debug)
    ...
    def single_image_pair_qualitative_analysis(self, dcn, dataset, rgb_a, rgb_b, mask_a, mask_b,
        ...
        for i in xrange(0, num_matches):
            ...
            best_match_uv, best_match_diff, norm_diffs =\
                self.Network.find_best_match(pixel_a, res_a,
                                             res_b)
    ...
    def compute_loss_on_dataset(self, dcn, data_loader, calc_losses_fn, num_iterations=500,):
        ...
        for i, sample in enumerate(data_loader, 0):
            ...
            with torch.no_grad():
                ...
                total_loss += loss.flatten()
                total_match_loss += match_loss.flatten()
                total_masked_non_match_loss += masked_non_match_loss.flatten()
                total_background_non_match_loss += background_non_match_loss.flatten()
                total_blind_non_match_loss += blind_non_match_loss.flatten()

        return total_loss / i, total_match_loss / i, total_masked_non_match_loss / i, total_background_non_match_loss / i, total_blind_non_match_loss / i
